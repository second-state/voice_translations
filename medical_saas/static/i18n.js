...
STRINGS.en = {
  ...
  'how.3.p': 'Each turn appears in both languages within a second or two, and can be read aloud in a different voice per side. What builds up is a complete record of the visit, which exports as a timestamped file.',
  ...
  'feat.privacy.p': 'The server stores no recordings and no transcripts. The record of the visit is yours: it lives in your browser tab until you clear or export it. What we do store is your email address and whether you are subscribed.',
  ...
  'record.h2': 'Nobody has to remember it all',
  'record.lede': 'Instructions given through an interpreter are the ones most easily lost on the way home. Every turn of the visit is written down as it happens — in both languages, with the time it was said — so the conversation can be read again when it matters.',
  'record.1.h': 'Both languages, side by side',
  'record.1.p': 'Each turn keeps what was said and how it was interpreted. A dose, a date, or an instruction can be checked against the original words rather than against someone’s memory of them.',
  'record.2.h': 'It outlives the appointment',
...
STRINGS.zh = {
  ...
  'how.3.h': '阅读或朗读',
  'how.3.p': '每句话在一两秒内以两种语言显示，并可用不同声音分别朗读双方的内容。逐渐积累成一份完整的就诊记录，可导出为带时间戳的文件。',
  ...
  'feat.privacy.p': '服务器不保存录音，也不保存转写记录。这份就诊记录属于你：它只存在于你的浏览器标签页中，直到你清除或导出。我们保存的只有你的邮箱地址和订阅状态。',
  ...
  'nav.record': '就诊记录',
  'record.h2': '不必全靠记忆',
  'record.lede': '经过口译传达的医嘱，最容易在回家路上丢失。就诊过程中的每一句话都会实时记录下来 — 两种语言并列，并标注说话时间 — 需要时可以重新读一遍。',
  'record.1.h': '两种语言并排呈现',
  'record.1.p': '每一句都同时保留原话与口译结果。剂量、日期或医嘱可以对照原话核实，而不是靠某个人的记忆。',
  'record.2.h': '就诊结束后依然在',
...
STRINGS.yue = {
  ...
  'how.3.h': '睇或者聽返',
  'how.3.p': '每句話一兩秒內就會用兩種語言顯示，仲可以用唔同聲線分別讀出雙方嘅內容。慢慢就儲成一份完整嘅應診紀錄，可以匯出成帶時間嘅檔案。',
  ...
  'feat.privacy.p': '伺服器唔會儲存錄音，亦唔會儲存轉寫紀錄。呢份應診紀錄係你嘅：佢只係存喺你個瀏覽器分頁，直到你清除或者匯出。我哋儲存嘅只有你嘅電郵地址同訂閱狀態。',
  ...
  'nav.record': '應診紀錄',
  'record.h2': '唔使全靠記',
  'record.lede': '經傳譯講出嚟嘅醫囑，最容易喺返屋企路上唔記得。應診期間每一句都會即時寫低 — 兩種語言並排，仲有講嘅時間 — 有需要嗰陣可以再睇一次。',
  'record.1.h': '兩種語言並排',
  'record.1.p': '每一句都同時保留原話同傳譯結果。劑量、日期或者醫囑都可以對返原話嚟核實，唔使靠邊個記唔記得。',
  'record.2.h': '應診完咗都仲喺度',
...
STRINGS.es = {
  ...
  'how.3.p': 'Cada intervención aparece en ambos idiomas en uno o dos segundos y puede leerse en voz alta con una voz distinta para cada lado. Lo que se acumula es un registro completo de la consulta, que se exporta como archivo con marcas de tiempo.',
  ...
  'feat.privacy.p': 'El servidor no almacena grabaciones ni transcripciones. El registro de la consulta es suyo: vive en la pestaña de su navegador hasta que lo borre o lo exporte. Lo que sí guardamos es su correo electrónico y si está suscrito.',
  ...
  'record.h2': 'Nadie tiene que recordarlo todo',
  'record.lede': 'Las indicaciones dadas a través de un intérprete son las que más fácilmente se pierden de camino a casa. Cada intervención de la consulta se escribe mientras ocurre —en los dos idiomas y con la hora en que se dijo—, de modo que la conversación puede releerse cuando hace falta.',
  'record.1.h': 'Los dos idiomas, uno junto al otro',
  'record.1.p': 'Cada intervención conserva lo que se dijo y cómo se interpretó. Una dosis, una fecha o una indicación pueden contrastarse con las palabras originales en lugar de con el recuerdo que alguien tenga de ellas.',
  'record.2.h': 'Sobrevive a la cita',
...
STRINGS.ko = {
  ...
  'how.3.h': '읽거나 들려줍니다',
  'how.3.p': '각 발화가 1~2초 안에 두 언어로 표시되고, 양쪽을 서로 다른 목소리로 읽어 줄 수 있습니다. 그렇게 쌓인 진료 기록 전체를 시간이 표시된 파일로 내보낼 수 있습니다.',
  ...
  'feat.privacy.p': '서버는 녹음도 대화 기록도 저장하지 않습니다. 진료 기록은 여러분의 것입니다: 지우거나 내보내기 전까지 브라우저 탭 안에만 있습니다. 저장하는 것은 이메일 주소와 구독 여부뿐입니다.',
  ...
  'nav.record': '진료 기록',
  'record.h2': '모두 기억할 필요는 없습니다',
  'record.lede': '통역을 거쳐 전달된 지시는 집에 가는 길에 가장 쉽게 사라집니다. 진료의 모든 발화가 오가는 즉시 기록됩니다 — 두 언어를 나란히, 말한 시각과 함께 — 그래서 필요할 때 다시 읽을 수 있습니다.',
  'record.1.h': '두 언어를 나란히',
  'record.1.p': '각 발화에 말한 내용과 통역 결과가 함께 남습니다. 용량이나 날짜, 지시 사항을 누군가의 기억이 아니라 원래의 말과 대조해 확인할 수 있습니다.',
  'record.2.h': '진료가 끝나도 남습니다',
...
STRINGS.ja = {
  ...
  'how.3.h': '読む、または聞く',
  'how.3.p': '各発話は一、二秒で両方の言語に表示され、双方を別々の声で読み上げられます。積み重なっていくのは診察の完全な記録で、タイムスタンプ付きのファイルとして書き出せます。',
  ...
  'feat.privacy.p': 'サーバーは録音も書き起こしも保存しません。診察の記録はあなたのものです：消すか書き出すまで、ブラウザのタブの中だけにあります。保存するのはメールアドレスと購読状態だけです。',
  ...
  'nav.record': '診察の記録',
  'record.h2': 'すべてを覚えておく必要はありません',
  'record.lede': '通訳を介して伝えられた指示は、帰り道でもっとも失われやすいものです。診察中の一つひとつの発話は、その場で書き留められます — 二つの言語を並べ、話された時刻とともに — 必要なときに読み返せるように。',
  'record.1.h': '二つの言語を並べて',
  'record.1.p': '各発話に、話された言葉とその通訳の両方が残ります。用量も日付も指示も、誰かの記憶ではなく元の言葉と照らして確かめられます。',
  'record.2.h': '診察が終わっても残ります',
