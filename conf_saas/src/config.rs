...
//! `[server] [audio] [languages] [asr] [llm] [tts]` belong to the
//! `voice_translations` library, `[conference]` to the `conf_translations`
//! translator, and `[auth] [email] [quota] [stripe] [admin]` to the
//! `saas_core` service layer. This module reads the file once and hands the
//! text to all three.
...
use conf_translations::config::ConferenceConfig;
...
pub struct AppConfig {
    ...
    /// The translator's own settings, shared with the standalone conference
    /// app.
